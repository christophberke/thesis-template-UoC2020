...
The resulting color arrays are shown in \figref{fig:colorspaces}. Note that the end colors are the same, but the intermediate colors depend on the colorspace in which we interpolate. The most striking feature is, that even though all colors from the HSV map have the same distance in the three-dimensional HSV space, the greenish colors are barely distinguishable, whereas the blueish colors are very distinct. This would constitute a very bad color map as it would lead to `flat' areas where differences between scalar values are not correctly resolved. To a lesser extend, the RGB colors suffer from a similar problem near the blue end.

Die Hauptaussage dieses kleinen Experiments ist: Ein vernuenftiger Abstand zwischen zwei Farben misst sich nicht in dem Abstand in einem Color Space, sondern darin wie das menschliche Auge und Gehirn Farbunterschiede interpretieren!
A good colormap is \emph{perceptually uniform}:
In a topographic elevation map, a change in altitude from 1500 meters below sea level to 1200 meters below sea level (i.e., a shift from dark blue to a lighter blue) should be interpreted as the same raise in height as a stept from 1000 meters above sea level to 1300 meters above sea level, i.e., (more tanned).
Just like a certain distance on the $x$ or $y$ axis of a regular 2D axis always means the step on the scale.

This allows us to specify our sofar only vague idea about a distance measure in color space: it should incorporate not only details about the actual colors but also about how these are processed by the human eye and brain. Unsurprisingly, this is a complicated issue, not fully understood and requires field studies.
...
We can have a look at the lightness of the ten colors of our maps and see whether our intuition (Lab is best, followed by RGB and then by HSV) was correct.

It is not a coincidence that Lab performs best: It is an example of a perceptually uniform color space that was specifically designed to allow for perceptual uniform colors by choosing regular points in a color space. The L turns out to be precisely the lightness (and the data in Fig. xx was created by converting all colors to Lab colors and then extract the `L' parameter.)
There are a variety of perceptual uniform color spaces, the first one being the CIE... , (where CIE stands for Commission internationale de l'éclairage ) defined in 1936 which was replaced by the imporved Lab and Luv spaces in 1976. 

 International Commission on Illumination calls


As our understanding of color perception matures, increasingly better metrics for a color distance (according to the CIE usually called $\Delta E^*$, where $E$ stands for the German word `Empfindung') become available. The first definition of $\Delta E^*$ (from 1976) is simply the Eucledian distance in the Lab color space.
This definition has been updated several times when it was discovered, that the CIELAB is not perfectly perceptual uniform.
CIELAB space turned out to be not as perceptually uniform as intended, especially in the saturated regions. 
The current definition is called CIEDE2000, usually abbreviated as $\Delta E^*_{00}$ and--when written in terms of the Lab parametrs becomes a pretty ugly thing.

Fig.dd does not only show the lightness but also the metrics ... 
